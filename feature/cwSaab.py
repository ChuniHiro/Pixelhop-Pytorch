...
class cwSaab():
    def __init__(self, depth=1, TH1=0.01, TH2=0.005, SaabArgs=None, shrinkArgs=None, load=False, rmcp=False):
        self.trained = False
        self.split = False
    ...
    @gc_invoker
    def SaabFit(self, X, layer, bias=0, rmcp=False):
        '''Learn a saab model'''
        shrinkArg, SaabArg = self.shrinkArgs[layer], self.SaabArgs[layer]
        assert ('func' in shrinkArg.keys()), "shrinkArg must contain key 'func'!"
        X = shrinkArg['func'](X, shrinkArg)
        S = list(X.shape)
        X = X.reshape(-1, S[-1])
        saab = Saab(num_kernels=SaabArg['num_AC_kernels'], needBias=SaabArg['needBias'], bias=bias, rmcp=rmcp)
        saab.fit(X)
        return saab
    ...
    def cwSaab_1_layer(self, X, train, bias=None):
        ...
        if self.SaabArgs[0]['cw'] == True: # channel-wise saab
            ...
            for i in range(X.shape[0]):
                ...
                if train == True:
                    # fit
                    saab = self.SaabFit(X_tmp, layer=0, rmcp=self.rmcp)
            ...
            if train == True:
                saab = self.SaabFit(X, layer=0, rmcp=self.rmcp)
                saab = self.discard_nodes(saab)
                saab_cur.append(saab)
                bias_cur.append(saab.Bias_current)
                eng.append(saab.Energy)
    ...
    def cwSaab_n_layer(self, X, train, layer):
        ...
        output, eng_cur, cov_cur, ct, pidx = [], [], [], -1, 0
        S = list(X.shape)
        ...
        if train == True:   
            if self.split == True or self.SaabArgs[0]['cw'] == False:
                ...
                self.cov['Layer' + str(layer)] = cov_cur
